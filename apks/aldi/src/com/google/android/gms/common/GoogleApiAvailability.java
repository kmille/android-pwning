  ...
  extends GoogleApiAvailabilityLight
{
  ...
  private final void zaa(Context paramContext, int paramInt, String paramString, PendingIntent paramPendingIntent)
  {
    ...
    Object localObject2 = ConnectionErrorMessages.getErrorNotificationTitle(paramContext, paramInt);
    paramString = ConnectionErrorMessages.getErrorNotificationMessage(paramContext, paramInt);
    Object localObject1 = paramContext.getResources();
    NotificationManager localNotificationManager = (NotificationManager)paramContext.getSystemService("notification");
    localObject2 = new NotificationCompat.Builder(paramContext).setLocalOnly(true).setAutoCancel(true).setContentTitle((CharSequence)localObject2).setStyle(new NotificationCompat.BigTextStyle().bigText(paramString));
    if (DeviceProperties.isWearable(paramContext))
    {
      Preconditions.checkState(PlatformVersion.isAtLeastKitKatWatch());
      ((NotificationCompat.Builder)localObject2).setSmallIcon(paramContext.getApplicationInfo().icon).setPriority(2);
      if (DeviceProperties.isWearableWithoutPlayStore(paramContext)) {
        ((NotificationCompat.Builder)localObject2).addAction(R.drawable.common_full_open_on_phone, ((Resources)localObject1).getString(R.string.common_open_on_phone), paramPendingIntent);
      } else {
        ((NotificationCompat.Builder)localObject2).setContentIntent(paramPendingIntent);
      }
    }
    else
    {
      ((NotificationCompat.Builder)localObject2).setSmallIcon(17301642).setTicker(((Resources)localObject1).getString(R.string.common_google_play_services_notification_ticker)).setWhen(System.currentTimeMillis()).setContentIntent(paramPendingIntent).setContentText(paramString);
    }
    if (PlatformVersion.isAtLeastO())
    {
      ...
      if (paramPendingIntent == null)
      {
        ...
        localObject1 = localNotificationManager.getNotificationChannel("com.google.android.gms.availability");
        paramContext = ConnectionErrorMessages.getDefaultNotificationChannelName(paramContext);
        if (localObject1 == null) {}
        for (paramContext = new NotificationChannel("com.google.android.gms.availability", paramContext, 4);; paramContext = (Context)localObject1)
        {
          localNotificationManager.createNotificationChannel(paramContext);
          paramString = paramPendingIntent;
          break;
          paramString = paramPendingIntent;
          if (paramContext.equals(((NotificationChannel)localObject1).getName())) {
            break;
          }
          ((NotificationChannel)localObject1).setName(paramContext);
        }
      }
      ((NotificationCompat.Builder)localObject2).setChannelId(paramString);
    }
    paramContext = ((NotificationCompat.Builder)localObject2).build();
    switch (paramInt)
    {
    default: 
      paramInt = 39789;
...
/* Location:              /home/kmille/projects/android-pwning/apks/aldi/ALDI TALK_v6.2.1_apkpure.com-dex2jar.jar!/com/google/android/gms/common/GoogleApiAvailability.class
