  ...
  implements RecyclerView.SmoothScroller.ScrollVectorProvider
{
  ...
  private void applyPendingSavedState(AnchorInfo paramAnchorInfo)
  {
      ...
      if (this.mPendingSavedState.mSpanOffsetsSize == this.mSpanCount)
      {
        ...
        while (i < this.mSpanCount)
        {
          ...
          if (k != Integer.MIN_VALUE)
          {
            ...
            for (j = this.mPrimaryOrientation.getEndAfterPadding();; j = this.mPrimaryOrientation.getStartAfterPadding())
            {
              j = k + j;
              break;
            }
    ...
    for (boolean bool = this.mPendingSavedState.mAnchorLayoutFromEnd;; bool = this.mShouldReverseLayout)
    {
  ...
  private boolean checkSpanForGap(Span paramSpan)
  {
    ...
    else if ((paramSpan.getStartLine() > this.mPrimaryOrientation.getStartAfterPadding()) && (!paramSpan.getLayoutParams((View)paramSpan.mViews.get(0)).mFullSpan)) {
      return true;
    }
    return false;
  }
  ...
  private int fill(RecyclerView.Recycler paramRecycler, LayoutState paramLayoutState, RecyclerView.State paramState)
  {
    ...
    updateAllRemainingSpans(paramLayoutState.mLayoutDirection, n);
    if (this.mShouldReverseLayout) {}
    for (int i1 = this.mPrimaryOrientation.getEndAfterPadding();; i1 = this.mPrimaryOrientation.getStartAfterPadding()) {
      break;
    }
    for (int i = 0; (paramLayoutState.hasMore(paramState)) && ((this.mLayoutState.mInfinite) || (!this.mRemainingSpans.isEmpty())); i = 1)
    {
      View localView = paramLayoutState.next(paramRecycler);
      LayoutParams localLayoutParams = (LayoutParams)localView.getLayoutParams();
      int i2 = localLayoutParams.getViewLayoutPosition();
      i = this.mLazySpanLookup.getSpan(i2);
      int k;
      if (i == -1) {
        k = 1;
      } else {
      ...
      if (paramLayoutState.mLayoutDirection == 1)
      {
        if (localLayoutParams.mFullSpan) {
          ...
          i = localSpan.getEndLine(i1);
        }
        j = this.mPrimaryOrientation.getDecoratedMeasurement(localView);
        if ((k != 0) && (localLayoutParams.mFullSpan))
        {
      ...
      else
      {
        if (localLayoutParams.mFullSpan) {
          ...
          i = localSpan.getStartLine(i1);
        }
        j = i - this.mPrimaryOrientation.getDecoratedMeasurement(localView);
        if ((k != 0) && (localLayoutParams.mFullSpan))
        {
      ...
      for (;;)
      {
        layoutDecoratedWithMargins(localView, i, j, m, i2);
        break;
        i2 = i;
        int i3 = m;
        i = j;
      ...
      if (localLayoutParams.mFullSpan) {
        updateAllRemainingSpans(this.mLayoutState.mLayoutDirection, n);
      } else {
        updateRemainingSpans(localSpan, this.mLayoutState.mLayoutDirection, n);
      }
  ...
  private void repositionToWrapContentIfNecessary()
  {
    ...
    while (i < m)
    {
      ...
      if (!localLayoutParams.mFullSpan)
      {
        if ((isLayoutRTL()) && (this.mOrientation == 1))
        {
          k = -(this.mSpanCount - 1 - localLayoutParams.mSpan.mIndex) * this.mSizePerSpan;
          j = -(this.mSpanCount - 1 - localLayoutParams.mSpan.mIndex) * n;
        }
        do
        {
          localView.offsetLeftAndRight(k - j);
          break;
          k = localLayoutParams.mSpan.mIndex * this.mSizePerSpan;
          j = localLayoutParams.mSpan.mIndex * n;
        } while (this.mOrientation == 1);
        localView.offsetTopAndBottom(k - j);
      }
      i += 1;
    }
  }
  ...
  public void collectAdjacentPrefetchPositions(int paramInt1, int paramInt2, RecyclerView.State paramState, RecyclerView.LayoutManager.LayoutPrefetchRegistry paramLayoutPrefetchRegistry)
  {
    ...
    if (getChildCount() != 0)
    {
      ...
      for (paramInt1 = 0; paramInt2 < this.mSpanCount; paramInt1 = i)
      {
        ...
        for (int j = this.mSpans[paramInt2].getStartLine(this.mLayoutState.mStartLine);; j = this.mLayoutState.mEndLine)
        {
          j = i - j;
          break;
  ...
  public int[] findFirstCompletelyVisibleItemPositions(int[] paramArrayOfInt)
  {
    ...
    label54:
    StringBuilder localStringBuilder = new StringBuilder("Provided int[]'s size must be more than or equal to span count. Expected:");
  ...
  public void onInitializeAccessibilityNodeInfoForItem(RecyclerView.Recycler paramRecycler, RecyclerView.State paramState, View paramView, AccessibilityNodeInfoCompat paramAccessibilityNodeInfoCompat)
  {
    ...
    paramAccessibilityNodeInfoCompat.setCollectionItemInfo(AccessibilityNodeInfoCompat.CollectionItemInfoCompat.obtain(i, j, k, m, paramRecycler.mFullSpan, false));
  }
        ...
        if (paramState != null)
        {
          ...
          if (this.mPendingScrollPositionOffset != Integer.MIN_VALUE)
          {
            if (paramAnchorInfo.mLayoutFromEnd) {
              i = this.mPrimaryOrientation.getEndAfterPadding() - this.mPendingScrollPositionOffset;
            }
            for (int j = this.mPrimaryOrientation.getDecoratedEnd(paramState);; j = this.mPrimaryOrientation.getDecoratedStart(paramState))
            {
              paramAnchorInfo.mOffset = (i - j);
              return true;
              i = this.mPrimaryOrientation.getStartAfterPadding() + this.mPendingScrollPositionOffset;
            }
          }
  ...
  class AnchorInfo
  {
    ...
    {
      if (this.mLayoutFromEnd) {}
      for (paramInt = StaggeredGridLayoutManager.this.mPrimaryOrientation.getEndAfterPadding() - paramInt;; paramInt = StaggeredGridLayoutManager.this.mPrimaryOrientation.getStartAfterPadding() + paramInt)
      {
  ...
  static class LazySpanLookup
  {
    ...
    void offsetForAddition(int paramInt1, int paramInt2)
    {
      if (this.mData != null)
      {
        ...
        ensureSize(i);
        int[] arrayOfInt = this.mData;
        System.arraycopy(arrayOfInt, paramInt1, arrayOfInt, i, this.mData.length - paramInt1 - paramInt2);
    ...
    void offsetForRemoval(int paramInt1, int paramInt2)
    {
      if (this.mData != null)
      {
        ...
        System.arraycopy(arrayOfInt1, i, arrayOfInt2, paramInt1, arrayOfInt2.length - paramInt1 - paramInt2);
  ...
  class Span
  {
    ...
    public int findFirstCompletelyVisibleItemPosition()
    {
      ...
      for (int j = -1;; j = this.mViews.size())
      {
        return findOneVisibleChild(i, j, true);
        i = 0;
      }
    }
    ...
    public int findFirstPartiallyVisibleItemPosition()
    {
      ...
      for (int j = -1;; j = this.mViews.size())
      {
        return findOnePartiallyVisibleChild(i, j, true);
        i = 0;
      }
    }
    ...
    public int findLastCompletelyVisibleItemPosition()
    {
      ...
      for (int j = this.mViews.size();; j = -1)
      {
        return findOneVisibleChild(i, j, true);
        i = this.mViews.size() - 1;
      }
...
/* Location:              /home/kmille/projects/android-pwning/apks/aldi/ALDI TALK_v6.2.1_apkpure.com-dex2jar.jar!/android/support/v7/widget/StaggeredGridLayoutManager.class
