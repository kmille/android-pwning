...
public class DiffUtil
{
  ...
  {
    public final int compare(DiffUtil.Snake paramAnonymousSnake1, DiffUtil.Snake paramAnonymousSnake2)
    {
      int i = paramAnonymousSnake1.x - paramAnonymousSnake2.x;
      if (i == 0) {
        return paramAnonymousSnake1.y - paramAnonymousSnake2.y;
      }
      return i;
    }
  };
  ...
  public static DiffResult calculateDiff(@NonNull Callback paramCallback, boolean paramBoolean)
  {
    ...
    while (!localArrayList2.isEmpty())
    {
      ...
      Snake localSnake = diffPartial(paramCallback, localRange2.oldListStart, localRange2.oldListEnd, localRange2.newListStart, localRange2.newListEnd, arrayOfInt1, arrayOfInt2, j);
      if (localSnake != null)
      {
        ...
        {
          i = localSnake.x;
          localRange1.oldListEnd = i;
        }
        for (i = localSnake.y;; i = localSnake.y - 1)
        {
          localRange1.newListEnd = i;
          break label336;
        ...
        if (localSnake.reverse)
        {
          if (localSnake.removal)
          {
            i = localSnake.x + localSnake.size + 1;
          }
          else
          {
            localRange2.oldListStart = (localSnake.x + localSnake.size);
            i = localSnake.y + localSnake.size + 1;
            break label442;
          }
        }
        else {
          i = localSnake.x + localSnake.size;
        }
        localRange2.oldListStart = i;
        i = localSnake.y + localSnake.size;
        label442:
        localRange2.newListStart = i;
      ...
      while (paramInt2 >= 0)
      {
        ...
        if (j != 0)
        {
          if ((j != 4) && (j != 8))
          {
            ...
            {
              paramList = new StringBuilder("unknown flag for pos ");
              paramList.append(i);
              paramList.append(" ");
...
/* Location:              /home/kmille/projects/android-pwning/apks/aldi/ALDI TALK_v6.2.1_apkpure.com-dex2jar.jar!/android/support/v7/util/DiffUtil.class
