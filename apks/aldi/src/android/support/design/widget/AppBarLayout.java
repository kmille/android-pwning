  ...
  extends LinearLayout
{
  ...
  int getDownNestedPreScrollRange()
  {
    ...
    while (j >= 0)
    {
      ...
      if ((n & 0x5) == 5)
      {
        int m = i + (localLayoutParams.topMargin + localLayoutParams.bottomMargin);
        if ((n & 0x8) != 0)
        {
        ...
        {
          if ((n & 0x2) != 0) {}
          for (i = ViewCompat.getMinimumHeight(localView);; i = getTopInset())
          {
            i = m + (k - i);
            break;
          }
    ...
    extends HeaderBehavior<T>
  {
    ...
    private boolean shouldJumpElevationState(CoordinatorLayout paramCoordinatorLayout, T paramT)
    {
      ...
      while (i < j)
      {
        ...
        if ((paramT instanceof AppBarLayout.ScrollingViewBehavior)) {
          return ((AppBarLayout.ScrollingViewBehavior)paramT).getOverlayTop() != 0;
        }
        i += 1;
      }
      return false;
    ...
    public boolean onLayoutChild(CoordinatorLayout paramCoordinatorLayout, T paramT, int paramInt)
    {
      ...
      if ((this.offsetToChildIndexOnLayout >= 0) && ((i & 0x8) == 0))
      {
        ...
        for (paramInt = ViewCompat.getMinimumHeight(localView) + paramT.getTopInset();; paramInt = Math.round(localView.getHeight() * this.offsetToChildIndexOnLayoutPerc)) {
          break;
        }
      ...
      extends AbsSavedState
    {
      ...
      {
        public final AppBarLayout.BaseBehavior.SavedState createFromParcel(Parcel paramAnonymousParcel)
        {
          return new AppBarLayout.BaseBehavior.SavedState(paramAnonymousParcel, null);
        }
        
        public final AppBarLayout.BaseBehavior.SavedState createFromParcel(Parcel paramAnonymousParcel, ClassLoader paramAnonymousClassLoader)
        {
          return new AppBarLayout.BaseBehavior.SavedState(paramAnonymousParcel, paramAnonymousClassLoader);
        }
        
        public final AppBarLayout.BaseBehavior.SavedState[] newArray(int paramAnonymousInt)
        {
...
/* Location:              /home/kmille/projects/android-pwning/apks/aldi/ALDI TALK_v6.2.1_apkpure.com-dex2jar.jar!/android/support/design/widget/AppBarLayout.class
