  ...
  extends FloatingActionButtonImpl
{
  ...
  void onDrawableStateChanged(int[] paramArrayOfInt)
  {
    ...
    {
      boolean bool = this.view.isEnabled();
      float f = 0.0F;
      if (bool)
      {
        ...
        {
          paramArrayOfInt = this.view;
          f = this.pressedTranslationZ;
        }
        for (;;)
        {
          paramArrayOfInt.setTranslationZ(f);
          return;
  ...
  {
    ShadowViewDelegate localShadowViewDelegate;
    if (this.shadowViewDelegate.isCompatPaddingEnabled())
    {
      this.insetDrawable = new InsetDrawable(this.rippleDrawable, paramRect.left, paramRect.top, paramRect.right, paramRect.bottom);
      localShadowViewDelegate = this.shadowViewDelegate;
    }
    for (paramRect = this.insetDrawable;; paramRect = this.rippleDrawable)
    {
      localShadowViewDelegate.setBackgroundDrawable(paramRect);
      return;
      localShadowViewDelegate = this.shadowViewDelegate;
    }
  }
...
/* Location:              /home/kmille/projects/android-pwning/apks/aldi/ALDI TALK_v6.2.1_apkpure.com-dex2jar.jar!/android/support/design/widget/FloatingActionButtonImplLollipop.class
