  ...
  extends PathMotion
{
  ...
  public Path getPath(float paramFloat1, float paramFloat2, float paramFloat3, float paramFloat4)
  {
    ...
    if (Math.abs(f1) < Math.abs(f2))
    {
      ...
      f3 = this.mMinimumVerticalTangent * f5;
    }
    for (float f4 = this.mMinimumVerticalTangent;; f4 = this.mMinimumHorizontalTangent)
    {
      f3 *= f4;
      break;
      f2 = f3 / (f1 * 2.0F);
      ...
      f3 = this.mMinimumHorizontalTangent * f5;
    }
    f4 = f7 - f2;
    float f8 = f6 - f1;
    f8 = f4 * f4 + f8 * f8;
    f4 = f5 * this.mMaximumTangent * this.mMaximumTangent;
    if (f8 >= f3) {
      if (f8 > f4) {
        f3 = f4;
      } else {
...
/* Location:              /home/kmille/projects/android-pwning/apks/aldi/ALDI TALK_v6.2.1_apkpure.com-dex2jar.jar!/android/support/transition/ArcMotion.class
