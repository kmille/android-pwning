  ...
  implements NestedScrollingChild2, NestedScrollingParent2, ScrollingView
{
      ...
      if (i != 0)
      {
        ...
        if (!dispatchNestedScroll(0, n, 0, i - n, null, 1))
        {
          ...
          if (i != 0)
          {
            ensureGlows();
            if ((k <= 0) && (m > 0)) {}
            for (EdgeEffect localEdgeEffect = this.mEdgeGlowTop;; localEdgeEffect = this.mEdgeGlowBottom)
            {
              localEdgeEffect.onAbsorb((int)this.mScroller.getCurrVelocity());
              break;
              if ((k < j) || (m >= j)) {
                break;
              }
  ...
  protected int computeScrollDeltaToGetChildRectOnScreen(Rect paramRect)
  {
    ...
    if ((paramRect.bottom > j) && (paramRect.top > i))
    {
      ...
      for (i = paramRect.top - i;; i = paramRect.bottom - j) {
        break;
      }
      ...
      for (;;)
      {
        ...
        do
        {
          for (;;)
          {
            ...
            {
              for (paramMotionEvent = this.mEdgeGlowBottom;; paramMotionEvent = this.mEdgeGlowTop)
              {
        ...
        } while (!this.mScroller.springBack(getScrollX(), getScrollY(), 0, 0, 0, getScrollRange()));
      }
    }
    if (getChildCount() == 0) {
      return false;
  ...
  public boolean pageScroll(int paramInt)
  {
    ...
    if (i != 0)
    {
      this.mTempRect.top = (getScrollY() + k);
      i = getChildCount();
      if (i <= 0) {
        break label151;
      }
    ...
    label151:
    this.mTempRect.bottom = (this.mTempRect.top + k);
    return scrollAndFocus(paramInt, this.mTempRect.top, this.mTempRect.bottom);
  }
...
/* Location:              /home/kmille/projects/android-pwning/apks/aldi/ALDI TALK_v6.2.1_apkpure.com-dex2jar.jar!/android/support/v4/widget/NestedScrollView.class
